...
c.ServerInstanceGr(sys.stdout,"yarn",8050,"ResourceManager",here)
c.ServerListenPromise(sys.stdout,thishost,"yarn",8050)
...
# 3 IP addresses: src 192.168.5.176:34987 dst 192.168.5.55:50010" and 192.168.7.210

now = time.time() # or parse .. or submit 0 for repeated event

who = "?"
what = "route write block";
why = "hadoop.hdfs.server.datanode.fsdataset.DataNode"; 
where = c.HereGr(sys.stdout,"NYC cloud") # assume 192.168.7.210?
how = "Receiving BP-1060243018-192.168.7.210"  # ? some invariant or coarse grain 
icontext = "hadoop hdfs service";

c.EventClue(sys.stdout,who,what,0,where,how,why,icontext);

c.RoleGr(sys.stdout,where,"hadoop data node",c.HostID("192.168.7.210"),"hadoop hdfs service")




#Example 3: 2017-01-20 15:44:02,029 INFO org.apache.hadoop.yarn.server.nodemanager.DefaultContainerExecutor: Initializing user crluser
