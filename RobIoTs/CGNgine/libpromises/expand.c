    ...
    */
    if (!NullIterators(iter_ctx))
    {
        do
        {
            ...
            {
            char default_handle[CF_MAXVARSIZE];
            
            snprintf(default_handle, CF_MAXVARSIZE, "%.250s_%s_%d", CanonifyName(pp->promiser), pp->parent_promise_type->parent_bundle->name, (int)pp->offset.line);
            PromiseAppendConstraint(pp, "handle", RvalNew(default_handle, RVAL_TYPE_SCALAR), true);
