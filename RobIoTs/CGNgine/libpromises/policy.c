...
Body *PolicyGetBody(const Policy *policy, const char *ns, const char *type, const char *name)
{
 for (size_t i = 0; i < SeqLength(policy->bodies); i++)
    ...
    bool type_match = (strcmp(bp->type, type) == 0);

    // Temporary workaround for implementing generalized aliases (precompile as pcode)
    
    if (!type_match && strcmp(type, "set_labels") == 0)
       {
       if (strcmp(bp->type, "classes") == 0)
          {
          type_match = true;
          }
       }
    else if (!type_match && strcmp(type, "classes") == 0)
       {
       if (strcmp(bp->type, "set_labels") == 0)
          {
          type_match = true;
          }
       }
    ...
    if (type_match && strcmp(body_symbol, name) == 0)
       {
       free(body_symbol);
...
int PromiseGetConstraintAsInt(const EvalContext *ctx, const char *lval, const Promise *pp)
{
    ...
    if (cp)
    {
        if (cp->rval.type != RVAL_TYPE_SCALAR)
        {
            Log(LOG_LEVEL_ERR, "Anomalous type mismatch - expected type for int constraint %s did not match internals", lval);
            PromiseRef(LOG_LEVEL_ERR, pp);
            FatalError(ctx, "Aborted");
        }
