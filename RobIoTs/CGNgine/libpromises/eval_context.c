...
void NotePromiseCompliance(EvalContext *ctx, const Promise * pp, PromiseState state, const char *reason);
...
static void EvalContextStackFrameAddSoft(EvalContext *ctx, const char *context, const char *tags)
{
 ...
 if (EvalContextHeapContainsSoft(ctx, frame.owner->ns, context))
    {
    Log(LOG_LEVEL_WARNING, "Private class '%s' in bundle '%s' shadows a global class - you should choose a different name to avoid conflicts", copy, frame.owner->name);
    }
...
bool IsDefinedClass(const EvalContext *ctx, const char *context)
{
 ...
 if (!res.result)
    ...
    {
    // controlled cast. None of these should modify EvalContext
    ExpressionValue r = EvalExpression(res.result, &EvalTokenAsClass, &EvalVarRef, (void *)ctx);
                                   ...
                                   PersistentClassPolicy policy, const char *tags)
{
 ...
 if (!OpenDB(&dbp, dbid_state))
    ...
    Log(LOG_LEVEL_ERR, "While persisting class, unable to open database at '%s' (OpenDB: %s)", db_path, GetErrorStr());
    free(db_path);
    return;
    }
                          ...
                          PersistentClassPolicy policy, ContextScope context_scope)
{
 ...
 for (const Rlist *rp = list; rp != NULL; rp = rp->next)
    ...
    if (persistence_ttl > 0)
       ...
       {
       Log(LOG_LEVEL_VERBOSE, "C:    + promise set label/class '%s'", classname);
...
static void UpdatePromiseComplianceStatus(EvalContext *ctx, PromiseResult status, const Promise *pp, const char *reason)
{
 ...
 NotePromiseCompliance(ctx, pp, compliance_status, reason);
}
...
void cfPS(EvalContext *ctx, LogLevel level, PromiseResult status, const Promise *pp, Attributes attr, const char *fmt, ...)
{
 ...
 UpdatePromiseComplianceStatus(ctx, status, pp, msg);
 free(msg);
}
...
void NotePromiseCompliance(EvalContext *ctx, const Promise * pp, PromiseState state, const char *reason)
{
 ...
 char *context = lctx->pctx->log_hook(lctx->pctx, EvalContextGetPass(ctx), "");
 const char *handle = PromiseGetHandle(pp);
 int len = strlen(context)+strlen(handle);
 char *key = xmalloc(len+2);

 for (sp = context; (*sp != '[') && (*sp != '\0'); sp++)
    {
    }
 * sp = '\0';

  // Context looks like /default/main/commands/'/bin/echox bla'
  // Reason is a string
 
 snprintf(key, len, "%s,%s", context, handle);
 snprintf(filename, CF_BUFSIZE, "%s/outputs/%s", CFWORKDIR, CanonifyName(key));
 ...
 WriteDB(dbp, key, &newe, sizeof(newe));
 CloseDB(dbp);
 free(context);
 free(key);
}
