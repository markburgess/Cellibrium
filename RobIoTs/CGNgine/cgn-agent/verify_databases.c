...
static PromiseResult VerifySQLPromise(EvalContext *ctx, Attributes a, const Promise *pp)
{
 ...
 a.database.db_connect_db = database;
...
static int CheckDatabaseSanity(Attributes a, const Promise *pp)
{
 ...
 if ((a.database.operation) && ((strcmp(a.database.operation, "delete") == 0) || (strcmp(a.database.operation, "drop") == 0)))
    ...
    if (pp->comment == NULL)
       {
       Log(LOG_LEVEL_ERR, "When specifying a delete/drop from an SQL database you must add a comment. Take a backup of the database before making this change. This is a highly destructive operation.");
       retval = false;
       }
    }
...
static PromiseResult KeepSQLPromise(char *database, char *table, EvalContext *ctx, Attributes a, const Promise *pp)
{
 ...
 if (!cfdb.connected)
    ...
    if ((a.database.operation) && (strcmp(a.database.operation, "create") != 0))
       {
       Log(LOG_LEVEL_VERBOSE, "Could not connect an existing database '%s' - check server configuration?", database);
 ...
 Log(LOG_LEVEL_VERBOSE, "Trying to make database '%s'", database);
...
/* Check change of existential constraints - only applies to the servers, not embedded dbs which are files */
 ...
 if (a.database.db_server_type != DATABASE_TYPE_SQLITE)
    ...
    if ((a.database.operation) && (strcmp(a.database.operation, "create") == 0))
       {
       CfConnectDB(&cfdb, a.database.db_server_type, a.database.db_server_host, a.database.db_server_owner,
                   a.database.db_server_password, NULL,a. database.db_directory);
 ...
 if (!cfdb.connected)
    ...
    if (VerifyTablePromise(ctx, &cfdb, database, table, a.database.columns, a, pp, &result))
       {
       cfPS(ctx, LOG_LEVEL_VERBOSE, PROMISE_RESULT_NOOP, pp, a, "Table '%s' is as promised", table);
       }
    else
       {
...
static int VerifyRowContent(EvalContext *ctx, CfdbConn *cfdb, ARG_UNUSED char *database, char *table_name, ARG_UNUSED Rlist *columns, Attributes a, const Promise *pp, PromiseResult *result)
{
 ...
 if (a.database.rows)
    ...
    for (Rlist *rp = a.database.rows; rp != NULL; rp=rp->next)
       ...
       if (where)
          {
          xasprintf(&check_query, "select %s from %s where %s", inserts, table_name, wheres);
          }
       else
          {
          xasprintf(&check_query, "select %s from %s", inserts, table_name);
          }

       if (where)
          {
          xasprintf(&insert_query, "insert into %s (%s,%s) values(%s,%s)", table_name, inserts, where_inserts, values, where_values);
          }
       else
          {
          xasprintf(&insert_query, "insert into %s (%s) values(%s)", table_name, inserts, values);
          }

       if (where)
          {
          xasprintf(&update_query, "update %s set %s where %s", table_name, updates, wheres);
          }
       else
          {
          xasprintf(&update_query, "update %s set %s", table_name, updates);
          }
       ...
       if (where)
          ...
          if (CfFetchRow(cfdb)) // Check that all the columns match in the returned data
             ...
             if (where)
                {
                // If there is conditional we are updating, not inserting
          ...
          {
          /* Exact combination does not exist in DB, but similar records may exist
             Leave them alone, and add a new one ...*/
          insert = true;
          update = false;
          }
       ...
       if (insert)
          ...
          if ((!DONTDO) && (a.transaction.action != cfa_warn))
             ...
             Log(LOG_LEVEL_VERBOSE, "WARNING - with no '=' qualifiers this promise cannot converge, and will add to the database each time the promise is kept");
...
static void ParseRow(EvalContext *ctx, char *row, Item **targets, Item **where, Attributes a, const Promise *pp, PromiseResult *result)
{
 ...
 for (char *sp = row; *sp != '\0'; sp += skip)
    ...
    sscanf(item, "%64[^=~, ] %3[^' ] %64[^, ]", name, operator, operand);
...
static void ConvergeSQLSchemas(EvalContext *ctx, CfdbConn *cfdb, ARG_UNUSED char *database, char *table_name, Rlist *schema, Rlist *policy, Attributes a, const Promise *pp, PromiseResult *result)
{
 for (Rlist *rpp = schema; rpp != NULL; rpp=rpp->next)
    {
    if (policy && !InTable(policy, rpp->val.item))
       {
       Log(LOG_LEVEL_VERBOSE, "The schema has a column that is not as promised: \"%s\"", (char *)rpp->val.item);
       //DeleteEntryFromSchema(rpp->val.item, cfdb);
       }
    else
       {
       Log(LOG_LEVEL_VERBOSE, "Table %s column \"%s\" is as promised", table_name, (char *)rpp->val.item);
       }
    }
 
 for (Rlist *rpp = policy; rpp != NULL; rpp=rpp->next)
    {
    if (schema && !InTable(schema, rpp->val.item))
       {
       Log(LOG_LEVEL_VERBOSE, "Table %s promises field \"%s\" but it is not there", table_name, (char *)rpp->val.item);
...
Rlist *QueryTableColumns(CfdbConn *cfdb, char *db_name, char *table_name, ARG_UNUSED Attributes a, ARG_UNUSED const Promise *pp)
{
 ...
 switch (cfdb->type)
    ...
    case DATABASE_TYPE_MYSQL:
    case DATABASE_TYPE_POSTGRES:
        snprintf(query, CF_MAXVARSIZE - 1, "SELECT column_name,data_type,character_maximum_length FROM information_schema.columns WHERE table_name ='%s' AND table_schema = '%s'", table_name, db_name);
        break;
 ...
 switch (cfdb->type)
    ...
    case DATABASE_TYPE_POSTGRES:
        ...
        // This only checks the column/table names, not types!!
        
        while (CfFetchRow(cfdb))
           {
           snprintf(row, CF_BUFSIZE, "%s %s", CfFetchColumn(cfdb, 0), CfFetchColumn(cfdb, 1));
