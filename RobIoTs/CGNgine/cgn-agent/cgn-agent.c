...
*/
    ...
    snprintf(filename, CF_BUFSIZE, "%s/CGNgine.%s.log", GetLogDir(), VSYSNAME.nodename);
    MapName(filename);
...
static PromiseResult KeepAgentPromise(EvalContext *ctx, const Promise *pp, ARG_UNUSED void *param)
{
    ...
    else if (strcmp("classes", pp->parent_promise_type->name) == 0 || strcmp("labels", pp->parent_promise_type->name) == 0)
    {
