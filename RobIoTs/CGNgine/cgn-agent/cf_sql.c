...
static DbSQLiteConn *CfConnectSQLiteDB(const char *path, const char *database)
{
 ...
 if (rc != SQLITE_OK)
    {
    Log(LOG_LEVEL_VERBOSE, "Failed to connect to existing SQLite database '%s'", sqlite3_errmsg(lc->conn));
...
static void CfNewQuerySQLiteDb(CfdbConn *c, const char *query)
{
 ...
 if (sqlite3_prepare_v2(lc->conn,  query, -1, &(lc->res), 0) == SQLITE_OK)
    ...
    {
    Log(LOG_LEVEL_VERBOSE, "Failed result for (%s) SQLite database '%s'", query, sqlite3_errmsg(lc->conn));
...
static DbMysqlConn *CfConnectMysqlDB(const char *host, const char *user, const char *password, const char *database)
{
 ...
 if (!mysql_real_connect(&c->conn, host, user, password, database, 0, NULL, 0))
    {
    Log(LOG_LEVEL_VERBOSE, "Failed to connect to existing MySQL database '%s'", mysql_error(&c->conn));
    free(c);
    return NULL;
    }
...
static DbPostgresqlConn *CfConnectPostgresqlDB(const char *host, const char *user, const char *password, const char *database)
{
 ...
 if (PQstatus(c->conn) == CONNECTION_BAD)
    {
    Log(LOG_LEVEL_VERBOSE, "Failed to connect to existing PostgreSQL database. (PQconnectdb: %s)", PQerrorMessage(c->conn));
    free(c);
    return NULL;
    }
