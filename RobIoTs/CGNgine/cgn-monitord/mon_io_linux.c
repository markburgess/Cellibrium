...
static int DiskSectorSize(const char *sysfsname)
{
 ...
 if (!(fh = fopen(sysfspath, "r")))
    ...
    if (fscanf(fh, "%d", &size) != 1)
       ...
       if (ferror(fh))
          {
          Log(LOG_LEVEL_ERR, "Unable to read sector size for '%s'. Assuming 512 bytes. (fscanf: %s)", sysfsname, GetErrorStr());
          }
       else
          {
...
static bool IsPartition(const char *sysfsname)
{
 ...
 if (errno != ENOENT)
    {
    Log(LOG_LEVEL_ERR, "Unable to detect whether block device %s is a partition. (access: %s)", sysfsname, GetErrorStr());
    }
...
{
 char sysfspath[CF_BUFSIZE]; /* "Stacked" as LVM or cryptfs volume */
 DIR *dir;
 ...
 if (errno)
    {
    Log(LOG_LEVEL_ERR, "Unable to read 'slaves' sysfs subdirectory for '%s'. (readdir: %s)", sysfsname, GetErrorStr());
    }
