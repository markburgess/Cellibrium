 ...
 for (dirp = DirRead(dirh); dirp != NULL; dirp = DirRead(dirh))
    ...
    for (count = 0; count < 4; count++)
       ...
       if (strstr(dirp->d_name, index))
          ...
          Log(LOG_LEVEL_VERBOSE, "[cpu%d] acpi temperature temp%d to %lf", count, count, temp);
          }
       }
    fclose(fp);
    }
...
static bool GetSysThermal(double *cf_this)
{
 ...
 for (count = 0; count < 4; count++)
    ...
    switch (count)
       {
       case 0:
           cf_this[ob_temp0] = temp/1000.0;
           break;
       case 1:
           cf_this[ob_temp1] = temp/1000.0;
           break;
       case 2:
           cf_this[ob_temp2] = temp/1000.0;
           break;
       case 3:
           cf_this[ob_temp3] = temp/1000.0;
           break;
       }

    Log(LOG_LEVEL_VERBOSE, "[cpu%d] sys/thermal temperature temp%d to %lf", count, count, temp/1000.0);
    fclose(fp);
    }
 ...
 for (ip = list; ip != NULL; ip = ip->next)
    ...
    for (count = 0; count < 4; count++)
       ...
       if (strncmp(ip->name, name, strlen(name)) == 0)
          ...
          Log(LOG_LEVEL_VERBOSE, "[cpu%d] LM sensor temperature temp%d to %lf", count, count, temp);
          }
       }
    }
