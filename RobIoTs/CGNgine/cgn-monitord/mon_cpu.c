...
void MonCPUGatherData(double *cf_this)
{
 ...
 while (!feof(fp))
    ...
    if (sscanf(buf, "%s%ld%ld%ld%ld%ld%ld%ld", cpuname, &userticks, &niceticks, &systemticks, &idle, &iowait, &irq, &softirq) != 8)
       {
       continue;
       }
    ...
    Log(LOG_LEVEL_VERBOSE, "[%d] %s=%d to %.1lf after %ld 100ths of a second ", count++, OBS[slot][1], slot, cf_this[slot], total_time);
    }
